...
class ImgPicker
{
	...
	function __construct($options = array(), $error_messages = array())
	{
		...
		$this->options = array(
			...
			'upload_url' => $_SERVER['DOCUMENT_ROOT'].'/img/',
	...
	protected function initialize()
	{
		...
		if (!extension_loaded('gd') || !function_exists('gd_info')) {
		    echo $this->error = $this->getErrorMessage('gd');exit;
            return false;
		}
	...
	protected function delete()
	{
		...
		$filepath = $this->getUploadPath($filename);
		$filename1 = basename(@$_POST['data']['Userid'].'.png');
		$filepath1 = $this->getUploadPath($filename1);
...
public function generateResponse1($response)
	{ // print_r($response);
    $ext=explode('.',$response->name);
	@rename($this->options['upload_dir'].$response->name,$this->options['upload_dir'].'avatar'.$_POST['data']['Userid'].'.'.$ext[1]);
	@rename($response->versions['avatar']['url'],$this->options['upload_dir'].$_POST['data']['Userid'].'.png');
	$response->name='avatar'.$_POST['data']['Userid'].'.'.$ext[1];
	$response->url=$this->options['upload_dir'].'avatar'.$_POST['data']['Userid'].'.'.$ext[1];
	$response->versions['avatar']['url']=$this->options['upload_dir'].$_POST['data']['Userid'].'.'.$ext[1];
	
	//echo 'avatar'.$_POST['data']['Userid'].'.png';
		echo json_encode($response);
	}
	/**
	 * 	Get error message
