 ...
 */
abstract class Am_Controller_Api_Table extends Am_Controller_Api
{
    ...
    protected function getNestedController($nest)
    {
        ...
        if (!empty($relation['file']))
            require_once APPLICATION_PATH . '/' . $relation['file'];
        ...
        $module->checkPermissions(Zend_Controller_Front::getInstance()->getRequest(), $nest, 
    ...
    protected function prepareRecordForDisplay(Am_Record $rec)
    {
        ...
        foreach ($nested as $nest)
        {
            ...
            /* @var $controller Am_Controller_Api_Table */
            $controller->setRequest(new Am_Request(array(
                '_filter' => array($keyField => $rec->pk()),
            )));
            $slaveRecords = $controller->selectRecords();
    ...
    public function selectRecords(& $total = 0)
    {
        $page = $this->_request->get('_page', 0);
        $count = $this->_request->get('_count', 20);
        $count = min(100, $count);
